...
- **Lagring**: Bruk alltid `entry.entry_id` som lagringsnøkkel, aldri `tso_id` eller brukervalgt konfigurasjon. Se [incident 001](docs/incidents/001-delt-data-mellom-instanser.md).
- **Satser**: Endringer i `const.py` (avgifter, terskel) eller `tso.py` (energiledd, kapasitetstrinn) krever offisiell kilde og bestått testsuite.
