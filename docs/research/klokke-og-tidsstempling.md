...
Dette er bekreftet, men det forklarer ikke avviket vårt:
...
## 10-sekunders-stemplingen er villet
...
Direkte sammenligning er klart best, og shift gjør det mye verre. Vår tpi-diff per time har altså riktig time-label. De 9 Wh som mangler over måneden er ren sample-presisjon fra at HAN-broadcast kommer 13 sek etter time-grensen, ikke et label-problem.
...
### Anbefaling for prosjektet

Behold dagens valg (aksepter 9 Wh som dokumentert sample-presisjon). Avviket er under BKKs egen avrunding og innenfor enhver praktisk relevans for fakturakontroll. Hvis 0 Wh-gap blir kritisk senere, implementer p-strøm-interpolering som opsjonelt flagg i export-scriptet.
