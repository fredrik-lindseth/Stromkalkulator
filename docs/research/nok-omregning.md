...
19 bankdager. Norges Bank publiserer middelkurs (mid-point i interbankmarkedet) basert på samme 14:15 CET-snapshot som ECBs euro-referansekurs, med publisering ~16:00 CET. Norges Bank synkroniserte publiseringstidspunktet sitt til ECBs etter ECB-omleggingen 1. juli 2016.

- Første (01.04): 11,2080 NOK/EUR
- Siste (30.04): 10,9123 NOK/EUR
- Aritmetisk snitt: 11,0229 NOK/EUR
- Trend: kronen styrket seg fra 11,21 til 10,91 i løpet av måneden
...
Vi gjorde det forrige notatet kalte overkill: hentet rå EUR/MWh-priser direkte fra Nord Pool (via hvakosterstrommen.no-speilet) og kjørte 10 NOK-omregnings-varianter mot fakturaen. Reproduserbar via `just verify-april` (eller `scripts/research/match_norgespris_variants.py --emit-markdown`).

Tabellen under regenereres automatisk av `just verify-all` (kun lokale fixturer, ingen internett). Manuell historisk versjon med Nord Pool EXR fra live HKS-kall ligger i [tidligere git-revisjon](https://github.com/) av denne filen.
...
1. ~~Hente historiske interbankkurser 12:00 CET fra Bloomberg~~. Gjort 2026-06-20, lukket ikke avviket. Se [bloomberg-verifisering.md](bloomberg-verifisering.md).
...
Tilfeldighet ville gitt blandet fortegn. Systematisk skjevhet i én retning forteller at BKK bruker en _høyere_ kurs enn NB 14:15. Det stemmer med 12:00 CET-hypotesen i akkurat denne perioden: kronen styrket seg gjennom 2026 (april gikk fra 11,21 til 10,91), og når kronen styrker seg gjennom dagen er 12:00-kursen høyere enn 14:15-kursen.
...
Bloomberg 12:00-kursen er nå testet. Hypotesen holdt ikke: avviket ble ikke
systematisk mindre, og restavviket forblir i ±0,2 %-båndet. Full rapport med
data, metode, forbeholdene om dataene og veien videre i
[bloomberg-verifisering.md](bloomberg-verifisering.md).
...
bare i april, så en ren tid-på-dagen-forklaring holder ikke. Restavviket
kommer mest sannsynlig fra Nord Pools egen publiserte NOK-pris (etter
to-banks-hedge) eller per-time-runding, ikke fra en FX-snapshot vi kan
rekonstruere utenfra.

## Variant-matrise per måned

Samme variantsett kjørt mot alle måneder vi har Norgespris-faktura for. Bekrefter at "beste variant"-valget ikke er stabilt over måneder, og at NB-kurser ligger innenfor ±0,3 % uavhengig av valgt forward/backward-fill-strategi.
