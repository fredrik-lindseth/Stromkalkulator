...
def _arkiv(sti: Path, timer: int = 72, nok_mwh: float = 1500.0) -> Path:
    """Kvarterarkiv med fire kvarter per time, stigende priser."""
    kvarter = []
    for i in range(timer * 4):
        kvarter.append({
            "start_local": _iso(START + timedelta(minutes=15 * i)),
            "nok_mwh": nok_mwh + i,
        })
    sti.write_text(
...
def test_time_00_langt_fra_forrige_kvarter_star_urort(tmp_path: Path) -> None:
    ...
    hours[24]["spot_nok_kwh_eks_mva"] = round(
        _kvarterpris(arkiv, f"2026-08-16T23:45:00{TZ}") + 0.01, 6
    )
